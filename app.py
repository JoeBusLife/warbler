...
def list_users():
    ...
    session["url"] = url_for('list_users', q=search)
...
def add_like(msg_id):
    """Like a message as the the currently-logged-in user."""
...
def remove_like(msg_id):
    """Unlike a message as the the currently-logged-in user."""
...
def homepage():
    ...
    if g.user:
        feed_ids = [f.id for f in g.user.following]
        feed_ids.append(g.user.id)
        
        messages = (Message
                    .query.filter(Message.user_id.in_(feed_ids))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
